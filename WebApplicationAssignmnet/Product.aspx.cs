    ...
    public partial class ProductPage : System.Web.UI.Page
    {
        ...
        private void GetProductDetails(int id)
        {
            try
            {
                ...
                {
                    conn.Open();
                    string query = @"select p.*, a.ArtFullName, a.CreatedAt from products p inner join Artist a on (a.ArtistID = p.ArtistID) where ProductID=@productID";
                    ...
                    if (reader.HasRows)
                    {
                        while (reader.Read())
                        {
                            ...
                            LblArtistName.Text = "By " + reader.GetString(11);
                            LblDate.Text = reader.GetDateTime(12).ToString("dd-MM-yyyy");
                        }
                    }
